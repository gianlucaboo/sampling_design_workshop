...
2.  Click on **Day 4: Introduction to Sampling Design**
...
ℹ️ The sample mean can be computed by substituting $N$ with $n$, while the sample variance by substituting $N$ with $n$ and $n-1$ in the denominator (Bessel’s correction).
...
**Sample mean** $$ 
    ...
    $$ **Population mean** $$ 
    \mu = \frac{1}{N} \sum_{h=1}^{H} \sum_{i=1}^{N_h} x_{h_{i}}
    $$ **Population variance** $$
      \sigma^2 = \frac{1}{N} \sum_{h=1}^{H} \sum_{i=1}^{N_h} (x_{h_{i}} - \mu)^2
    $$
:::
...
ℹ️ The mean and variance are weighted by the stratum's size ($W_h=\dfrac{N_h}{N}$).
...
$$ **Approximate variance of the sample mean** $$
...
ℹ️ The mean and variance are weighted by the clusters' size similar to stratified sampling ($W_h=\dfrac{N_h}{N}$) if needed.
...
**Population total (Horvitz–Thompson estimator)** $$
\hat T = \sum_{i\in S} \frac{x_i}{\pi_i}
$$

**Population mean (Horvitz–Thompson estimator)** $$
\mu_{ht} = \frac{1}{N} \sum_{i \in S} \frac{x_i}{\pi_i}
$$ ℹ️ The Horvitz–Thompson estimator works if $N$ is known.
:::
...
$\mu_{HT}$: Population mean\
$\hat T$: Population total\
$\pi_i$: Inclusion probability for for unit $i$\
$w_i$: Sampling weight for unit $i$, $1/\pi_i$\
:::
:::::
...
-   Common design:
    -   Stage 1: Randomly sampled **clusters** (e.g., schools).
    -   Stage 2: Randomly sampled **elements within clusters** (e.g., students).
...
5.  Minimize **bias and sampling error**
...
Särndal, C.-E., Swensson, B., & Wretman, J. (1992). *Model Assisted Survey Sampling*. Springer. https://d-nb.info/969712979/04

Lohr, S. L. (2019). *Sampling: Design and Analysis* (2nd edition). Chapman & Hall/CRC. https://www.taylorfrancis.com/books/mono/10.1201/9780429296284/sampling-sharon-lohr

Lumley, T. (2010). *Complex Surveys: A Guide to Analysis Using R*. Wiley. https://onlinelibrary.wiley.com/doi/book/10.1002/9780470580066

Zimmer, S. A., Powell, R. J., & Velásquez I. C. (2025). *Exploring complex survey data analysis using R*. https://tidy-survey-r.github.io/tidy-survey-book/index.html

Lumley, T., & Scott, A. (2023). *survey: Analysis of Complex Survey Samples*. R package version 4.2-3. https://cran.r-project.org/web/packages/survey/index.html
